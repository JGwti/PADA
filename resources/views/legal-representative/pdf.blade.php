    ...
    <table class="table-head" CELLPADDING=10 CELLSPACING=0>
        <TR>
            ...
            <TD class="table-head-ms" ROWSPAN=2>
                {{$legalRepresentative->form->typeForm->name_form}}
            </TD>
            <TD class="table-head-ms"> Código: </TD>
            <TD class="table-head-ms">
                {{$legalRepresentative->form->typeForm->code_form}}
            </TD>
        </TR>
        <TR>
            <TD class="table-head-ms">
        ...
        <div class="contenedor">
            <p class="p-justificado text-size">
                ...
                <samp class="line-document">
                    {{$legalRepresentative->representative_document_number}}
                </samp>
                de
                <samp class="line-name">
                    {{ $legalRepresentative->representative_document_city }}
                </samp>
                ...
                @if ( $studentReferral->patient->documentType->name == 'Registro civil')
                    x
                @endif
                ...
                @if ( $legalRepresentative->professional_area=='psychology')
                    x
                @endif
                ) y/o psicopedagogía (

                @if ( $legalRepresentative->professional_area == 'psychopedagogy')
                    x
                @endif
                ), con
                ...
                <samp class="line-date-day">
                    {{$legalRepresentative->day}}
                </samp>
                días del mes de
                <samp class="line-date-month">
                    {{ $legalRepresentative->mont}}
                </samp>
                del año
                <samp class="line-date-year">
                    {{$legalRepresentative->year}}
                </samp>
                , en la ciudad de
                <samp class="line-city">
            ...
            <table class="table">
                <tr>
                    {{-- firma de paciente --}}
                    <th class="table">
                        <p><img src="{{ public_path('./storage/signatures/'.$legalRepresentative->patient_signature) }} " width="100" height="70"></p>
                    </th>
                    {{-- firma representante --}}
                    <th class="table">
                        <p><img src="{{ public_path('./storage/signatures/'.$legalRepresentative->representative_signature) }}" width="100" height="70"></p>
                    </th>
                </tr>
                <tr class="table table-font">
                    <th class="table">Firma de quien recibirá el acompañamiento</th>
                    <th class="table">Firma del representante legal</th>
                </tr>
                <tr>
                    {{-- nombre de paciente --}}
